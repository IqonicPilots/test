...
export function useThemeManager() {
  ...
  const resetTheme = React.useCallback(() => {
    ...
    const allPossibleVars = [
      ...
      'ring',
      // NOTE: 'radius' is intentionally excluded — it is managed independently by applyRadius()
      // and must persist across theme preset changes.
    ...
    // --radius is intentionally preserved — it is managed by applyRadius() independently.
    const inlineStyles = root.style
    for (let i = inlineStyles.length - 1; i >= 0; i--) {
      const property = inlineStyles[i]
      if (property.startsWith('--') && property !== '--radius') {
  ...
  const applyTheme = React.useCallback((themeValue: string, darkMode: boolean) => {
    ...
    // Re-apply any existing brand color overrides
    Object.entries(brandColorsValues).forEach(([cssVar, value]) => {
      if (value) root.style.setProperty(cssVar, value)
    })
  }, [resetTheme, brandColorsValues])
  ...
  const applyCustomColors = React.useCallback((colors: Record<string, string>) => {
    resetTheme()
    const root = document.documentElement
